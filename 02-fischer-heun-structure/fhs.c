...
size_t fhs_query_summary(struct fhs_t *fhs, size_t block_i, size_t block_j) {
  ...
  size_t second_half = fhs->summary[(size_log2 - 1) * fhs->num_blocks + block_i + half_span];
...
size_t fhs_query(struct fhs_t *fhs, size_t i, size_t j) {
  ...
  if (full_block_i < full_block_j) {
    minimum = fhs_query_summary(fhs, full_block_i, full_block_j);
  }
  if (block_i != full_block_i) {
    size_t first_block_minimum = fhs_query_block(fhs, block_i, i % fhs->block_size, fhs->block_size);
    if (fhs->arr[first_block_minimum] < fhs->arr[minimum]) {
      minimum = first_block_minimum;
    }
  }
...
int main() {
  ...
  struct fhs_t *fhs = fhs_preprocess(arr, sizeof(arr) / sizeof(int));

  fhs_assert(fhs, 0, 0, 0);
  fhs_assert(fhs, 1, 0, 0);
  fhs_assert(fhs, 0, 31, 31);
